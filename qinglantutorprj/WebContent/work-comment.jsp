...
<body>
	...
	<c:forEach items="${homework}" var="h">
		...
		<div  style="width: 800px;height: 110px;float: left;overflow-y:auto;overflow-x: hidden;
			...
			<c:if test="${not empty ts}">
				<div id="time1" > 
					...
					<div  class="t2" ><input type="text" style="width:40px;"  name="score" disabled="disabled">  </div>
		...
		<!-- 	作业评语表单提交 -->
				...
				<textarea style="resize: none;width: 430px;height: 125px;margin-left:-80px ;margin-top: 40px;float: left;border:1px dashed #c0c0c0;" name="comment" disabled="disabled"></textarea>
				<input type="submit" value="提交"  class="btn-submit" style="float: right;margin-right: 47px;margin-top: -28px;"  disabled="disabled"/>
			</c:if>
			<c:if test="${ empty ts}">
				<div id="time1" > 
					<div  class="c7" style="height: 20px;width: 75px;">分数</div>
