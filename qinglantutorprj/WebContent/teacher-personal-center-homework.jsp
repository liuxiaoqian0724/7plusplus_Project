        ...
        <div class="panel-body" style="position:relative;">
        	...
        	<c:forEach  items="${homework }" var="h">
          	 <div>
           	 ...
           	 <c:if test="${h.getClassRelation().getUser().getId()==s.id }">
           	
              <div class="c2-right-left">
                  <img src="${s.userImg }" width="115px" height="115px" class="img-circle" alt="圆形图片">
                  <p>${s.userName}</p>
                  <button>${h.tstatus}</button>
              </div>
               </c:if>
              </c:forEach>
              <div class="c2-right-right">
                  <div>
                      <h3>任务一 力学</h3>
                      <p>${fn:substring(h.scontent, 0, 10)}...</p>
                      <span>${h.sfinishTime}</span>
                  </div>
                  <button type="button"><a href="WorkComment?hid=${h.hid}">查看详情</a></button>
                   <button type="button"><c:if test="${h.tstatus == '未评价'}"><a href="WorkComment?hid=${h.hid}">未评价</a></c:if><c:if test="${h.tstatus == '已评价'}">已评价</c:if></button>
              </div>
           </div>
           </c:forEach>
