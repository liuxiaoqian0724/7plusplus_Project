...
import com.sevenpp.qinglantutor.entity.Grade;
		...
		*/
		@Repository
		public class ConditionsDaoImpl implements TutorListDao {
			@Resource
			...
			public List<Grade> findGradesBySchoolType(String schoolType) {
				...
				if(schoolType.equals("xiaoxue")) {	//显示小学的年级
					q=session.createQuery("from Grade where schooltype=:schooltype");
					q.setString("schooltype", "小学");
				}else if(schoolType.equals("chuzhong")) {	//显示初中的年级
					q=session.createQuery("from Grade where schooltype=:schooltype");
					q.setString("schooltype", "初中");
				}else if(schoolType.equals("gaozhong")) {		//显示高中的年级
					q=session.createQuery("from Grade where schooltype=:schooltype");
