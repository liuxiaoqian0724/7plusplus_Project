...
public class TutorAsCandidateController {
	...
	public void sendMsgToTutor(@RequestParam("arr") String arr,HttpServletRequest request,HttpServletResponse response,
		...
		if(!digitList.isEmpty()) {
			...
			JFreeChart jfreechart = new JFreeChart("Compare", TextTitle.DEFAULT_FONT, spiderwebplot, false);
			...
			String img=  "<img src='" 
		            ...
		            + filename + "'/>"; 
			model.addAttribute("radarimage", img);
			request.setAttribute("radarimage", img);
			session.setAttribute("radarimage", img);
